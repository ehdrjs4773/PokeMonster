...
{
	if (SOUNDMANAGER->isPlaySound("레드의 테마곡"))SOUNDMANAGER->stop("레드의 테마곡");
	stageManager::update();
	collision();
	_gymLeaderRc = RectMake(_x, _y, _gymLeader->getWidth(), _gymLeader->getHeight());
	...
	if (_isWin)
	{
		...
		if (SOUNDMANAGER->isPlaySound("배틀씬 테마곡")) SOUNDMANAGER->stop("배틀씬 테마곡");
		if (SOUNDMANAGER->isPlaySound("레드의 테마곡")) SOUNDMANAGER->stop("레드의 테마곡");
		if (SOUNDMANAGER->isPlaySound("오박사 테마곡")) SOUNDMANAGER->stop("오박사 테마곡");
		if (SOUNDMANAGER->isPlaySound("월드맵 테마곡")) SOUNDMANAGER->stop("월드맵 테마곡");

		SOUNDMANAGER->play("엔딩곡" , 1.0f);
	}
}
...
void champion::collision()
{
	...
	if (IntersectRect(&temp, &_gymLeaderRc, &_player->getPlayerRc()) && _isWin == false)
	{
		...
		{
			// 여기랑
			SOUNDMANAGER->play("레드의 테마곡", 1.0f);
